...
const express = require('express'),
    ...
    mqtt = require('mqtt'),
    {parseTopicsFromDevice, powerTopicSuffix, stateTopicSuffix, bitwiseTurnPowerOnOrOffByState} = require('./mindoMQTT'),
    {v4} = require('uuid');
...
let getDevicesStateFromServer = function (callback) {
    lastUpdate = Date.now();
    makeRequest('https://webapp.mindolife.com/API/Gateway/getDeviceUnites', undefined, callback);
};

let handleGetDevicesState = function (res) {
    // check if cached
...
const updateDeviceState = function (deviceId, bitwiseState) {
    ...
    makeRequest("https://webapp.mindolife.com/API/Gateway/changeControlState", 
        ...
        function (error, response, body) {
            if (!!error) {
                log(`Got error while updating:  ${err}`);
            }
        }
    );
};

let deviceStatesByTopic = {};
...
const bootstrapDevices = function (client) {
    ...
    getDevicesStateFromServer(function(err, body) {
        ...
        try {
            ...
            if (!devicesArray) {
                log(`No devices were found in JSON: ${JSON.stringify(devicesJSON)}`);
                return;
            }
            ...
            for (const topic of Object.keys(newState)) {
                client.subscribe(topic, function (err) {
                    if (!err) {
                        log(`Succseefully subscribed to topic ${topic}`);
                    }
                });
            }
...
const digestMessage = function(topic, message) {
    ...
    if (deviceStatesByTopic[topic] != message) {
        ...
        if (topic.endsWith(powerTopicSuffix)) {
            const lastKnownState = deviceStatesByTopic[topic.replace(powerTopicSuffix, stateTopicSuffix)];
...
app.listen(3000, function () {
    ...
    client.on('connect', function () {
        ...
        poller = setInterval(pollAndUpdate, config.pollingInterval, client);
